...
# The name of the file to be compiled. Will be dropped in root.
OUTFILE := program
...
TEST_SRC_FILES := $(shell find $(TEST_SRC_DIRS) -name '*c') $(filter-out $(MAIN_C), $(SRC_FILES))
...
# Build the program executable (after building .o files)
$(OUTFILE): $(OBJ_FILES)
	$(CC) $(OBJ_FILES) -o $@
